...
const Summary = () => {
  ...
  useEffect(async()=>{
    ...
    if(date!=="" && to!==""){
        setLoader(true)
        let resp=await getSummary(token,date,to);
    if(resp){
  ...
  return (
    <div className="m-2 md:m-10 mt-24 p-2 md:p-10 bg-white rounded-3xl">
    ...
    </div>:<div style={{width:"100%",display:"flex",justifyContent:"space-around",flexWrap:'wrap'}}>
      ...
      <OperatorSummary dateFrom={date} to={to}/>
      </div>
      }
    ...
    {date!="" && to!=""?<div style={{display:'flex',alignItems:"center",alignContent:"center",justifyContent:"space-between",width:'100%',flexWrap:"wrap"}}><ColorMapping dateFrom={date} to={to}/><Line line={line} setLine={setLine} dateProp={date} to={to}/></div>:null}
    {date!="" && to!=""?<div style={{display:'flex',alignItems:"center",alignContent:"center",justifyContent:"space-between",width:'100%',flexWrap:"wrap"}}><Stacked setStacked={setStacked} to={to} dateProp={date}/><Pie to={to} dateProp={date}/></div>:null}
